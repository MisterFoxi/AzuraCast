...
final readonly class PutMembersAction implements SingleActionInterface
{
    ...
    public function __invoke(
        ...
        // Validate all members before making any changes, so we never leave a group half-updated.
        ...
        // We fully recreate all memberships of the group on every save rather than diffing/
        // updating existing rows. Simpler and safer given groups are typically small.
        ...
        foreach ($members as $member) {
            ...
            // Weight is derived from array order (drag-and-drop reorder on the frontend sends
            // members in the desired play order); an explicit 'weight' still overrides it.
            $stationPlaylistGroup->weight = isset($member['weight'])
                ? Types::int($member['weight'])
                : $weight;
